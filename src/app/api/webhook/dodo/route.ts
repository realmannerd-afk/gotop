...
export const POST = async (req: NextRequest) => {
  try {
    ...
    if (!secret) {
      console.error('CRITICAL: DODO_WEBHOOK_KEY is not configured on the server.');
      return new Response('Webhook secret not configured.', { status: 500 });
    }

    const payload = await Webhooks(req, {
      secret,
      onPayload: async (payload) => {
        ...
        if (payload.type === 'payment.succeeded') {
          ...
          // Log failed payment attempt directly in payments table for forensic record
          if (!process.env.SUPABASE_SERVICE_ROLE_KEY) throw new Error('Missing service role key');
          const supabaseAdmin = createClient(
            process.env.NEXT_PUBLIC_SUPABASE_URL!,
            process.env.SUPABASE_SERVICE_ROLE_KEY,
